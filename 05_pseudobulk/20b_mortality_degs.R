...
DIR <- '/projects/b1196/ewa_group/serniczek/data/05_pseudobulk/20a_alive_vs_dead'
...
process_cell_type <- function(meta_path, labels_path, count_path, output_path) {
  ...
  for (sample_group in unique(sample_groups$group)) {
    if (sample_group %in% c('Healthy', 'NPC', 0, '0')) {
      CTRL <- sample_group
    } else {
      EXPR <- sample_group
    }
...
if (length(args) < 8) {
  stop("Usage: Rscript 20b_mortality_degs.R --meta <m> --count <c> --labels <l> --output <o>")
}
