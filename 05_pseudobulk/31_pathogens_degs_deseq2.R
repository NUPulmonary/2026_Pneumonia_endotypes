...
SHORTCUTS_REV <- split(names(SHORTCUTS), unlist(SHORTCUTS))
...
process_cell_type <- function(meta_path, labels_path, count_path, output_path) {
  ...
  task_info <- unlist(strsplit(task, '_vs_'))
  CTRL <- SHORTCUTS_REV[[task_info[1]]]
  EXPR <- SHORTCUTS_REV[[task_info[2]]]
...
if (length(args) < 8) {
  stop("Usage: Rscript 31_pathogens_degs_deseq2.R --meta <m> --count <c> --labels <l> --output <o>")
}
