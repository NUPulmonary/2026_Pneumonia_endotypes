...
# Isolate the sample ids of the pathogen negative samples
sc_labels = pd.read_csv(common_data.SC_LABELS)
pathogen_negative = list(sc_labels.bal_barcode[sc_labels.is_culture_negative_pneumonia.fillna(False)])

def filter_split_data(data):
    task_column = task
    idx = []
    for individual in data['individual']:
        idx.append(individual in pathogen_negative)
